...
class Artist(object):
    ...
    def __repr__(self):
        return repr({'name': self.name, 'url': self.url, 'mbid': self.mbid,
                     'upcomming_event_count': self.upcomming_event_count})
...
class Event(object):
    ...
    def __repr__(self):
        return repr({'id': self.id, 'url': self.url, 'ticket_url': self.ticket_url, 'ticket_status': self.ticket_status,
                     'datetime': self.datetime, 'on_date_datetime': self.on_date_datetime, 'artists': self.artists,
                     'venue': self.venue})
    ...
    def parse(cls, data):
        ...
        return Event(data.get('id'), data.get('url'), data.get('ticket_url'), data.get('ticket_status'),
                     data.get('datetime'), data.get('on_date_datetime'), artists, venue, data.get('status'))

    @classmethod
    def generate_args(cls, mbids=None, artists=None, location=None, radius=None, date='upcoming', page=None,
                      per_page=None):
    ...
    @classmethod
    def recommended(cls, mbids=[], artists=[], location=None, radius=None, date=None, only_recs=None, page=None,
                    per_page=None):
        """
...
class Venue(object):
    ...
    def __repr__(self):
        return repr(
                {'id': self.id, 'name': self.name, 'city': self.city, 'region': self.region, 'country': self.country,
                 'url': self.url, 'latitude': self.latitude, 'longitude': self.longitude})
    ...
    @classmethod
    def parse(cls, data):
        return Venue(data.get('id'), data.get('name'), data.get('city'), data.get('region'), data.get('country'),
